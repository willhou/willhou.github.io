...
struct MeetPhoneSpec {
    let source: String
    let output: String
    let statusBarBackground: NSColor?
    let statusBarInk: NSColor?
    let sourceRightCrop: CGFloat

    init(
        source: String,
        output: String,
        statusBarBackground: NSColor? = nil,
        statusBarInk: NSColor? = nil,
        sourceRightCrop: CGFloat = 0
    ) {
        self.source = source
        self.output = output
        self.statusBarBackground = statusBarBackground
        self.statusBarInk = statusBarInk
        self.sourceRightCrop = sourceRightCrop
    }
}
...
func composeMeetPhone(_ spec: MeetPhoneSpec) throws {
    guard
        let sourceImage = NSImage(contentsOfFile: spec.source),
        let source = sourceImage.cgImage(
            forProposedRect: nil,
    ...
    let systemBarBackground = spec.statusBarBackground ?? NSColor(
        srgbRed: 247 / 255,
        green: 247 / 255,
        blue: 247 / 255,
        alpha: 1
    )
    let systemBarInk = spec.statusBarInk ?? NSColor(
        srgbRed: 113 / 255,
        green: 122 / 255,
        blue: 128 / 255,
        alpha: 1
    ...
    guard let bitmap = NSBitmapImageRep(
        ...
        throw NSError(
            domain: "ComposeSunriseDevices",
            code: 5,
            userInfo: [NSLocalizedDescriptionKey: "Could not create Meet phone bitmap"]
        )
    }
    ...
    sourceImage.draw(
        ...
        from: NSRect(
            x: 0,
            y: 0,
            width: sourceImage.size.width - spec.sourceRightCrop,
            height: sourceImage.size.height
        ),
        operation: .sourceOver,
        fraction: 1,
    ...
    guard let data = bitmap.representation(using: .png, properties: [:]) else {
        throw NSError(
            domain: "ComposeSunriseDevices",
            code: 6,
            userInfo: [NSLocalizedDescriptionKey: "Could not encode Meet phone PNG"]
        )
    }
    try data.write(to: URL(fileURLWithPath: spec.output))
}
...
let meetPhoneSpecs = [
    MeetPhoneSpec(
        source: "\(projectImages)/sunrise-meet-compose-screen.jpg",
        output: "\(projectImages)/sunrise-device-meet-compose.png",
        statusBarBackground: .white,
        statusBarInk: NSColor(
            srgbRed: 113 / 255,
            green: 122 / 255,
            blue: 128 / 255,
            alpha: 1
    ...
    MeetPhoneSpec(
        source: "\(projectImages)/sunrise-meet-invite.png",
        output: "\(projectImages)/sunrise-device-meet-invite.png"
    ),
    MeetPhoneSpec(
        source: "\(projectImages)/sunrise-meet-picker.png",
        output: "\(projectImages)/sunrise-device-meet-picker.png"
    ),
]

for spec in meetPhoneSpecs {
    try composeMeetPhone(spec)
}
